...
class _ManagePageState extends State<ManagePage> {
  ...
  Widget _buildPartitionWithItems(List<FridgeItem> items) {
    final config = fridgeLayouts[_selectedFridge]?[_compartments[_currentCompartment]];
  ...
  void _showItemDetailDialog(FridgeItem item) {
    final aliasCtrl    = TextEditingController(text: item.ingredient_id.toString());
    final nameCtrl     = TextEditingController(text: item.id);
    final qtyCtrl      = TextEditingController(text: '1');
    final categoryCtrl = TextEditingController(text: '');
    final boughtCtrl   = TextEditingController(text: DateFormat('yyyy-MM-dd').format(DateTime.now()));
    final expireCtrl   = TextEditingController(text: DateFormat('yyyy-MM-dd').format(DateTime.now().add(Duration(days:7))));
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              TextFormField(controller: aliasCtrl,    decoration: const InputDecoration(labelText: 'Alias')),
              TextFormField(controller: nameCtrl,     decoration: const InputDecoration(labelText: '식재료 명')),
              TextFormField(controller: qtyCtrl,      keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: '수량')),
              TextFormField(controller: categoryCtrl, decoration: const InputDecoration(labelText: '카테고리')),
              TextFormField(controller: boughtCtrl,   decoration: const InputDecoration(labelText: '구매일자 (YYYY-MM-DD)')),
              TextFormField(controller: expireCtrl,   decoration: const InputDecoration(labelText: '소비기한 (YYYY-MM-DD)')),
              TextFormField(controller: memoCtrl,     decoration: const InputDecoration(labelText: '메모'), maxLines: 2),
              TextFormField(controller: areaCtrl,     decoration: const InputDecoration(labelText: 'Area ID')),
            ],
          ),
        ),
        actions: [
  ...
  Future<void> _updateItemOnServer(FridgeItem item, Map<String, dynamic> data) async {
    final uri = Uri.parse('https://your.api.com/ingredients/${item.ingredient_id}');
